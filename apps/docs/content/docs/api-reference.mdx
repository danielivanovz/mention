...
| `Mention.Popover` | Caret-positioned listbox, hidden after a failed search. Accepts div props, `maxHeight` (280 by default), and `container`. `container={null}` renders in place. |
