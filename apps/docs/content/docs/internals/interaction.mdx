...
Options are not additional tab stops by default. The primary mouse-down handler prevents the browser's focus change and attempts to commit the selected item. Mouse movement can update the highlight; a stationary pointer does not steal the initial highlight when the popup appears beneath it.
