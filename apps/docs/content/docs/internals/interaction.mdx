...
A failed request hides the popup and omits `aria-controls`, while preserving the failed session for an explicit retry. The [async search example](/docs/recipes/async-items) supplies a persistent status region and a recovery button outside the listbox. The application owns their wording; the library does not insert extra announcements during option navigation.
