...
Mention is a headless **React 19** library for suggestions at the caret. These examples require Mention 0.2: install `@danielivanov/mention@^0.2.0` with the application's package manager. Import `@danielivanov/mention/styles.css` once if you want the default popup styles.
