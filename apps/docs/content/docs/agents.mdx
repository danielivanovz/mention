...
Arrays are filtered locally by case-insensitive label substring. An async fetcher has type `(query: string, signal: AbortSignal) => Promise<readonly Person[]>`; it supplies its own search results. Forward `signal` to network calls and keep the fetcher reference stable. The default debounce is 150 ms. The core also rejects obsolete results when a fetcher ignores cancellation.
