...
- The AI composer is a third lazy playground mode and an executable integration guide. It uses the shared Lexical host for document reference IDs, shadcn/ui for controls and conversation layout, AI SDK 7 for transport, and `@shadcn/helpers` for explicitly scripted local responses. A request inspector shows submitted message parts; a one-shot failure control demonstrates retained drafts and retry. No live model endpoint is added to the website.
- A root GitHub registry item installs the same composer, host, message types, and server reference resolver used by the examples. The item declares Mention as an npm dependency and excludes helpers and demo data. Public availability depends on publishing its declared Mention version and merging the registry source. No approved shadcn directory namespace is claimed.
