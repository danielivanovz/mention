...
- [The GitHub registry](registry.json) installs the AI SDK 7 + shadcn/ui composer from `packages/react/examples/registry/default/ai-composer`. That directory is the shared source for the registry, website, and browser fixtures; its Lexical host also powers the ordinary Lexical example. Keep `@shadcn/helpers` and sample data in `AIComposerDemo.tsx`, outside the installable item. Validate with `bunx --bun shadcn@4.21.0 registry validate registry.json`; verify actual installation in a clean React 19 shadcn app after changing registry dependencies, targets, or imports. Public installation requires the declared Mention npm version to exist.
...
AI context references come from one current editor snapshot, never accumulated `onSelect` callbacks. The server resolver validates message parts and resolves IDs through an application-owned authenticated lookup before converting custom data into model context. The starter accepts text and user document references; extending it to tools requires the matching schemas. AI SDK and shadcn stay consumer/example dependencies, outside Mention's runtime package.

Results belong to their request session. Reject obsolete results and insertion into a changed document or selection. Derive synchronous filtering during render. A dismissal applies to the unchanged snapshot. Channels with `allowSpaces` also keep forward typing from the same dismissed prefix closed; earlier edits, a new trigger, or explicit opening can make it eligible again. Synchronous array matching can be supplied with a pure `filter`; async results bypass it. Failed requests hide the popup; an explicit open retries the failed query, while ordinary refreshes preserve it. Keep recovery controls and application-owned status messages outside the listbox.
