...
bun run size           # size-limit, enforces 14 kB cap (ADR-0007)
...
- **ADR-0001**: Option C — own ARIA layer + Floating UI + vendored caret math. Rejected Ariakit and Base UI Combobox (dynamic role mutation breaks the contract).
...
- **Trigger detection** (`packages/react/src/state/find-active-mention.ts`): single source of truth. Pure `(value, caret, trigger) → ActiveMention | null`. Reducer is a pure transition machine reflecting the dispatcher's result (ADR-0003).
...
- **Per-slice commits**: one slice = one commit, roadmap row ID in subject. e.g. `feat(react): vendor textarea caret math (I2)`.
...
- **Releases**: Changesets (wired when L6 approaches).
...
Dual: npm package + parallel shadcn-cli registry (L4, blocked on AT pass). Default CSS uses shadcn-compatible variables so the registry path drops in cleanly.
